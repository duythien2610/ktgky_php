<x-laptop-layout>
    ...
    <div style="background:#fff; padding:15px 0; margin-bottom:15px; display:flex; justify-content:center; align-items:center; gap:12px;">
        ...
        <a href="{{ url('/') }}?{{ http_build_query(array_merge(request()->query(), ['sort' => 'asc'])) }}"
           style="background:#fff; border:1px solid #ccc; padding:6px 15px; border-radius:4px; color:#333; text-decoration:none; font-size:0.9rem; <?php echo (isset($sort) && $sort === 'asc') ? 'border-color:#28a745; color:#28a745;' : ''; ?>">
            Giá tăng dần
        </a>
        <a href="{{ url('/') }}?{{ http_build_query(array_merge(request()->query(), ['sort' => 'desc'])) }}"
           style="background:#fff; border:1px solid #ccc; padding:6px 15px; border-radius:4px; color:#333; text-decoration:none; font-size:0.9rem; <?php echo (isset($sort) && $sort === 'desc') ? 'border-color:#28a745; color:#28a745;' : ''; ?>">
            Giá giảm dần
        </a>
        @if(isset($sort) && $sort)
